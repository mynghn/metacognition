...
- **Consumer→vault references are soft, and externally checked.** A skill that points into the vault fuses three layers — an inline floor that works with the vault absent, its own local procedure, and an *optional* pointer to a single canonical entry for depth. That pointer is a soft reference, never a hard import: a renamed or missing entry degrades only the optional depth, never the consumer's own output. Nothing else watches these pointers, so `check-refs` is their sole external guard — it reports any that no longer resolves but never makes a consumer fail at runtime. Keeping the link soft is what keeps vault evolution cheap: a hard dependency would make every rename break its consumers, *raising* the cost of the evolution this two-repo layout exists to make safe.
