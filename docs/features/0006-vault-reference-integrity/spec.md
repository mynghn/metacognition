...
Every covered consumer→vault reference is a soft pointer, never a hard dependency. A consumer's inline floor stays fully functional when the vault is absent or its referenced entry is missing — a missing entry degrades only the optional depth, never the consumer's own output. The check is the sole asserter of reference resolution: it reports a broken reference but never causes a consumer to fail at runtime. This is the observable form of the soft-reference contract whose intent Requirements owns.
