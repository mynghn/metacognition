...
- **Slot.** A named config the driver applies before running the corpus. Baseline `C0` = current state, no change — which now already carries main's trigger-first KB wiring (`29c45d0`; `UNDERSTANDING#Delta-1-baseline-now-ships-trigger-first-wiring`).
- **Initial candidate matrix (test inputs, not choices):** `C1` a standing skill-consultation checkpoint in the shared operating frame; `C2` a reflection/self-check pass over the just-finished turn; `C3` per-skill description variants — including the **capability-first** form as the contrast against the now trigger-first `C0` baseline, so the matrix adjudicates whether `29c45d0` earned its keep (`UNDERSTANDING#Delta-1-baseline-now-ships-trigger-first-wiring`); `C4` (control ceiling) the deterministic keyword reminder on the keyword-*present* subset — a reference ceiling, not a no-cue solution.
...
- **Schema (stdlib JSON; not YAML-for-data, per repo idiom):** `{ "id", "skill": "compact-focus|handoff", "setup": "<what creates the triggering condition + the no-cue user turn>", "framing": "deliberation|imperative|tangential|unrelated", "should_fire": true|false }`.
...
- **Realization.** A thin wrapper over headless `claude` (non-interactive print mode), seeding each scenario's `setup`; it records the transcript paths it produced for the scorer.
- **Reproducibility boundary.** Some triggers — e.g. compact-focus's "context is heavy" — are harder to reproduce headlessly than an explicit turn. A scenario whose triggering condition cannot be reproduced deterministically is dropped, not faked; the dropped set is logged, not hidden.
