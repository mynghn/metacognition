...
Produce a single ready-to-paste `/compact <focus>` command line that compacts THIS conversation well. Ground it in the context-engineering knowledge base's compaction discipline — load `@VAULT@/context-engineering/knowledge/compaction-vs-eviction.md` and its siblings for the full reasoning (that KB entry is the authority, not any one session's global-instructions block).
...
A finished line looks like this (content illustrative — name the real items of the session):
