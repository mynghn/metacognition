...
**Goal first.** The new goal is the organizing input. Infer it from the user's request, or ask for it before writing — selection is meaningless without a destination. If the "new goal" is really "keep doing the same thing," stop and recommend the `compact-focus` skill instead (continuity, not departure).

**Gate.** Hand off only if a fresh frame beats staying: execution needs a cold load this session lacks, the session has rotted, or it is a clean explore→execute boundary. If everything the goal needs is already warm and near-continuous, say so and suggest staying / the `compact-focus` skill — don't manufacture a handoff.
